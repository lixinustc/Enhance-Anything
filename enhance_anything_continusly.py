...
import CResMD.options as option
import utils.util as util
from CResMD.data.util import bgr2ycbcr
from CResMD.data import create_dataset, create_dataloader
from CResMD.models import create_model
...
if __name__ == "__main__":
    ...
    mask_dir = Path(args.output_dir+'/mask')/img_stem
    mask_dir.mkdir(parents=True, exist_ok=True)
    out_fore_dir = Path(args.output_dir+'/fore')/img_stem
    ...
    for test_loader in test_loaders:
        ...
        # print("======", img_stem)
        dataset_dir = osp.join('./results_continuous', img_stem)
        util.mkdir(dataset_dir)
        need_GT = False if test_loader.dataset.opt['dataroot_GT'] is None else True

        for coef in np.arange(start_point, 0.6, stride):
    ...
    # os.system('rm -rf {}'.format(out_fore_dir))
